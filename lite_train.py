...
gitFIB Dataset (Fish Instance Benchmark)
...
class FishKeypointDataset(Dataset):
    ...
    def _make_heatmap(self, kp_x, kp_y, visible):
        """Return [K, Hm, Wm] float32 heatmaps."""
        K  = cfg.NUM_KEYPOINTS
        ...
        for y in range(size):
            for x in range(size):
                g[y, x] = math.exp(-((x - c)**2 + (y - c)**2) /
                                    (2 * self.sigma ** 2))

        for k in range(K):
            if visible[k] == 0:
                continue
            cx = int(round(kp_x[k] * Wm))
            cy = int(round(kp_y[k] * Hm))
            if cx < 0 or cy < 0 or cx >= Wm or cy >= Hm:
                continue

            ul = [cx - c, cy - c]
            br = [cx + c + 1, cy + c + 1]

            g_xl = max(0, -ul[0]);  g_xr = size - max(0, br[0] - Wm)
            g_yl = max(0, -ul[1]);  g_yr = size - max(0, br[1] - Hm)
            h_xl = max(0, ul[0]);   h_xr = min(Wm, br[0])
            h_yl = max(0, ul[1]);   h_yr = min(Hm, br[1])

            heatmaps[k, h_yl:h_yr, h_xl:h_xr] = g[g_yl:g_yr, g_xl:g_xr]
